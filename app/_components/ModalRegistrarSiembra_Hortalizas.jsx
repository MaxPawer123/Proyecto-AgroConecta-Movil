...
import { crearLoteApi } from '@/src/services/api';
...
export default function ModalRegistrarSiembra_Hortalizas({ visible, onClose, onCreated }) {
  ...
  const crearLote = async () => {
    ...
    try {
      ...
      Alert.alert('Listo', 'Lote registrado en backend y guardado localmente.');
      limpiarFormulario();
      if (onCreated) {
        await onCreated();
      }
      ...
      >
        <View style={styles.modalContainer}>
          ...
          <ScrollView style={styles.scrollContent} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.fieldContainer}>
              <Text style={styles.label}>Tipo de Hortaliza</Text>
              <TouchableOpacity style={styles.selectInput} onPress={() => setModalTipoOpen(true)}>
                <Text style={form.tipoCultivo ? styles.selectValue : styles.selectPlaceholder}>
                  {form.tipoCultivo || 'Selecciona el tipo de hortaliza'}
                </Text>
      ...
      >
        <View style={styles.selectorOverlay}>
          <View style={styles.selectorModal}>
            <Text style={styles.selectorTitle}>Tipo de Hortaliza</Text>
            {TIPOS_HORTALIZA.map((tipo) => (
              <TouchableOpacity
                key={tipo}
